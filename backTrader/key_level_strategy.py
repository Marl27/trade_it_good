...
class Key_level_strategy(backtrader.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Submitted, order.Accepted]:
            self.log('ORDER ACCEPTED/SUBMITTED', self.data.num2date(dt=order.created.dt))
            self.order = order
    ...
    def next(self):
        ...
        if not self.position:
            if self.dataclose[0] < self.keys[0]:
                print("No trade, Share price too LOW {0} - {1} - {2}".format(
                    self.dataclose[0], current_bar_datetime, self.keys[0]))
                #return
            elif self.dataclose[0] > self.keys[-1]:
                print("No trade, Share price too HIGH {0} - {1} - {2}".format(
                ...
                print("close price {0} -- limitBuyPrice {1}".format(self.data.close[0], newlist[-1]))
                self.order = self.buy(exectype=backtrader.Order.Limit, price=newlist[-1], valid=datetime.now() + timedelta(minutes=1))
                bought_price = float(newlist[-1])  # setting bought_price value, relates to above question

        elif self.dataclose[0] >= round((bought_price + (0.05 * bought_price)), 5): #0.01
            # print("Profit selling {0}--{1}".format(
            # self.dataclose[0], round((bought_price + (0.005 * bought_price)), 5)))
            self.order = self.sell()
        elif self.dataclose[0] <= round((bought_price - (0.001 * bought_price)), 5):
